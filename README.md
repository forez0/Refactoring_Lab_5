...
---
## Налаштування CI/CD
## 🔁 CI/CD: Автоматизація перевірок через PR Checks

### 🎯 Мета автоматизації
CI/CD (Continuous Integration/Continuous Delivery) — це підхід до автоматизації перевірки, тестування та розгортання змін у коді. У цьому проєкті реалізовано перевірку коду за допомогою **GitHub Actions** через **PR Checks**, що забезпечує надійність змін перед їх об'єднанням у основну гілку.

---

### ⚙️ Покрокове налаштування GitHub Actions для Pull Request Checks

#### 🔹 1. Створення workflow-файлу
  ...
  validate:
    ...
    steps:
      ...
      - name: Run Tests
        run: pytest
``` </code></pre>
#### 🔹 2. Що виконується під час PR
Крок	Опис
✅ Checkout code	Отримання коду гілки, по якій відкрито PR
✅ Setup Python	Встановлення версії Python 3.11
✅ Install dependencies	Встановлення бібліотек з requirements.txt
✅ Run Tests	Запуск тестів з pytest для перевірки функціональності

#### 🔹 3. Результати у Pull Request
Після кожного коміту в PR:

GitHub запускає перевірку автоматично.

У вкладці Checks видно статус виконання: ✅ успіх або ❌ помилка.

Це забезпечує автоматичну перевірку коду перед злиттям у main.

🧪 Результат

✅ Автоматичне тестування при кожному PR

✅ Мінімізація ризику появи багів у main

✅ Прозора історія перевірок

✅ Підтримка високої якості коду
